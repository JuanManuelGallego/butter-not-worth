...
def main():
    ...
    z_bh, p_bh, k_bh = hp.paratf(z_pbas, p_pbas, k_pbas * -1 * K1, z_phaut, p_phaut, k_phaut * -1)

    # Passe Bande
    z_bande, p_bande, k_bande = hp.seriestf(z1, p1, k1, z2, p2, k2)

    # Total
    z, p, k = hp.paratf(z_bh, p_bh, k_bh, z_bande, p_bande, k_bande * K2)
    b, a = signal.zpk2tf(z, p, k)
    mag1, ph1, w1, fig, ax = hp.bodeplot(b, a, "Bode Équalizateur")
    hp.pzmap1(z, p, "PZ Passe Bande")
    temp = signal.lsim((z_bande, p_bande, k_bande), U=wave, T=t)

    delay = - np.diff(ph1) / np.diff(w1)
    hp.grpdel1(w1, delay, 'Delais')
    hp.timeplt1(t, wave, temp[0], temp[1], 'Signal Sinus 2.5kHz filtré')

    print(f'Équalizateur Numérateur {b}, Dénominateur {a}')
    print(f'Équalizateur Zéros {z}, Pôles {p}, Gain {k}')
